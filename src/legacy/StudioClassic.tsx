...
export default function StudioClassic() {
  ...
  // Qualité du rendu, choisie génération par génération.
  //
  // `isPro` ne suit plus l'abonnement : c'est le choix de l'utilisateur pour ce
  // rendu-ci. Il pilote à la fois le modèle (nano-banana-2-edit contre
  // nano-banana-pro-edit) et la variante de prompt. Le mode Pro est ouvert à
  // tous — c'est le prix en tokens qui le régule, pas un droit d'accès : un
  // compte gratuit qui dépense ses 10 tokens offerts en Pro nous coûte 0,27 $,
  // contre 0,25 $ en Standard. Le fermer ne protégerait rien et priverait le
  // prospect de la seule démonstration qui vend l'abonnement.
  const [isPro, setIsPro] = useState(false);

  // Le choix survit au redémarrage : un abonné qui travaille en Pro ne doit pas
  // le réarmer à chaque lancement.
  useEffect(() => {
      AsyncStorage.getItem('render_quality')
        .then(v => { if (v === 'pro') setIsPro(true); })
        .catch(() => {});
  }, []);

  const handleQualityChange = useCallback((pro: boolean) => {
      setIsPro(pro);
      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
      AsyncStorage.setItem('render_quality', pro ? 'pro' : 'standard').catch(() => {});
  }, []);

  const renderCost = isPro
    ? (entitlements.render_costs?.pro ?? 3)
    : (entitlements.render_costs?.standard ?? 1);

  // Un compte tout neuf n'a pas encore de ligne d'entitlement : remaining_total
  // vaut 0 alors que ses 10 tokens offerts l'attendent, octroyés par
  // reserve_generation à la première génération. Lui afficher « Get more
  // tokens » l'empêcherait définitivement de les recevoir — la seule fonction
  // qui les octroie ne serait jamais atteinte. On laisse donc passer tant
  // qu'aucune génération n'a eu lieu.
  const hasEverGenerated = (entitlements.monthly_usage ?? 0) > 0
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (activeMode === 'paint' && preparedSources.length >= 1) {
        ...
        images = await generatePaintedMiniature(
          preparedSources, finalPrompt, 1, creativityLevel, metadata,
          characterDesc, isPro ? 'pro' : 'standard');
      }
      ...
      <View style={styles.container}>
        ...
        <View style={[styles.footerContainer, { paddingBottom: Math.max(insets.bottom + 16, 24), paddingTop: sourceImages.length === 0 ? 32 : 8 }]}>
          <View style={styles.footerInner}>
            {/* Qualité du rendu. Placée juste au-dessus du bouton Créer, avec
                son prix : c'est ici que le choix se fait et qu'il se paie. */}
            {sourceImages.length > 0 && !isLoading && (
              <View style={styles.qualityRow}>
                {([
                  { pro: false, label: 'Standard', cost: entitlements.render_costs?.standard ?? 1 },
                  { pro: true,  label: 'Pro',      cost: entitlements.render_costs?.pro ?? 3 },
                ]).map((opt) => {
                  const active = isPro === opt.pro;
                  return (
                    <TouchableOpacity
                      key={opt.label}
